...
public class Card : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerEnterHandler,
  ...
  private static WaitForSecondsRealtime _waitForSecondsRealtime0_01 = new WaitForSecondsRealtime(time: 0.01f);
  ...
  [Header("拖拽与对齐设置")] [Tooltip("卡牌跟随速度")]
  public float followSpeed;

  [Tooltip("卡牌Y轴对齐距离")] public float yAlignedDistance = 40.0f;

  [Range(min: 1.0f, max: 1.2f)] [Tooltip("卡牌拖动时放大系数")]
  public float dragScaleFactor = 1.05f;

  protected Vector3 initPosition;

  [Header("描边设置")] public Material outlineMaterial;
  ...
  [Header("合成指示")] public GameObject craftTooltipPanel;
  ...
  [Header("卡牌设置")] public CardDescription cardDescription;
  ...
  public int durability {
    get => cardDescription.cardType switch {
        CardType.Resources => CardManager.Instance.GetCardAttribute<CardAttributeDB.ResourceCardAttribute>(cardID)
            .durability,
  ...
  public void SetCardType(CardDescription description) {
  ...
  public static bool CanPlacedOn(Card card, Card cardToPlace) {
    ...
    var basicCheck = cardToPlace.canBePlaced == true &&
  ...
  protected void Start() {
    ...
    asInputText.text = input == "" ? "无" : input.TrimEnd(trimChar: '\n');
    asOutputText.text = output == "" ? "无" : output.TrimEnd(trimChar: '\n');
  }
  ...
  protected void Update() {
    ...
    if (craftTooltipPanel.activeSelf && Input.GetMouseButtonDown(button: 0)) {
      ...
      var isPointerOver = RectTransformUtility.RectangleContainsScreenPoint(rect, Input.mousePosition, cam);
      if (!isPointerOver) {
        craftTooltipPanel.SetActive(value: false);
      }
    }
  ...
  protected void FollowPosition() {
    var targetPosition = preCard.transform.position - new Vector3(x: 0, yAlignedDistance, z: 0);
    ...
    if (Vector3.Distance(transform.position, targetPosition) <= 2f && !CardManager.Instance.isDragging &&
        !preCard.isMoving) {
      // Debug.Log($"Card {cardDescription} reached target position.");
      transform.position = targetPosition;
      ChangeMovingState(state: false);
    }
  }
  ...
  public void OnDrag(PointerEventData eventData) {
    ...
    RectTransformUtility.ScreenPointToWorldPointInRectangle(canvas.GetComponent<RectTransform>(), eventData.position,
        eventData.pressEventCamera, out var mousePos);
  ...
  public void OnEndDrag(PointerEventData eventData) {
    ...
    if (eventData.pointerCurrentRaycast.gameObject != null) {
      ...
      if (hitObject.CompareTag("Card")) {
        ...
        var card = hitObject.GetComponent<Card>();
        // Can place on the card
        if (CanPlacedOn(this, card)) {
          transform.position = card.transform.position - new Vector3(x: 0, yAlignedDistance, z: 0);
          CardSlot.ChangeCardsToSlot(cardSlot, card.cardSlot, GetFollowingCards(), card, controlNewCardPos: false);
        }
        else // Put to original cardSlot or create a new slot
        {
          transform.position = initPosition - new Vector3(x: 0, yAlignedDistance, z: 0);
          CardSlot.ChangeCardsToSlot(cardSlot,
              preCardBeforeDrag == null
                  ? CardManager.Instance.CreateCardSlot(initPosition)
                  : preCardBeforeDrag.cardSlot, GetFollowingCards(), preCardBeforeDrag, controlNewCardPos: false);
        }
      }
    }
    ...
    if (endOnCard == false) {
      ...
      var tmp = CardManager.Instance.CreateCardSlot(transform.position);
      CardSlot.ChangeCardsToSlot(cardSlot, tmp, GetFollowingCards(), afterCard: null, controlNewCardPos: false);
    }
  ...
  public void ChangeMovingState(bool state) {
    ...
    if (state == true) {
      ...
      setShadow();
      if (laterCard != null) {
        laterCard.ChangeMovingState(state: true);
      }
    }
    else {
      setShadow();
  ...
  public void OnPointerEnter(PointerEventData eventData) {
    var draggingCard = CardManager.Instance.draggingCard;
  ...
  public void OnPointerClick(PointerEventData eventData) {
    ...
    switch (cardDescription.cardType) {
      case CardType.Resources:
        craftTooltipPanel.SetActive(value: true);
        if (activeCraftTooltipPanel != null && activeCraftTooltipPanel != craftTooltipPanel) {
          activeCraftTooltipPanel.SetActive(value: false);
        }
  ...
  public void StartEvent(int optionIndex, float progress = 0f, EventUI eventUI = null) {
    ...
    cardSlot.StartProgressBar(eventUIAttribute.options[optionIndex].cost.timeCost, () => { OnEndEvent(eventUI); },
  ...
  public void OnEndEvent(EventUI eventUI) {
    ...
    foreach (var reward in eventUIAttribute.options[curWorkOptionIndex].rewards) {
      for (var i = 0; i < reward.dropCount; i++) {
        var randomOffset = UnityEngine.Random.insideUnitCircle * 50f;
