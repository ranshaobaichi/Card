...
public class BattleWorldManager : MonoBehaviour {
  ...
  private List<B_Obj> m_tickRemoveList;
  ...
  private void Awake() {
    ...
    OnTick = new EventActionDictionary<LineUp>();
    m_tickRemoveList = new List<B_Obj>();
  ...
  private void InvokeTick() {
    ...
    m_tickRemoveList.Clear();
    foreach (var (_, bObjs) in battleObjs) {
      foreach (var bObj in bObjs) {
        if (bObj is IBattleDamageable damageableObj && damageableObj.IsDead()) {
          m_tickRemoveList.Add(bObj);
        }
      }
    }
    foreach (var obj in m_tickRemoveList) {
      RemoveObj(obj);
    }
