...
public class CreatureAttributeDisplay : MonoBehaviour, IDragHandler, IBeginDragHandler {
  ...
  public void OnDrag(PointerEventData eventData) {
    RectTransformUtility.ScreenPointToWorldPointInRectangle(_canvas.GetComponent<RectTransform>(), eventData.position,
  ...
  public void UpdateAttributes(CardAttributeDB.CreatureCardAttribute creatureAttribute,
      CardAttributeDB.CreatureCardAttribute.BasicAttributes basicAttributes = null) {
    if (preDisplayPanel != null) {
      Destroy(preDisplayPanel);
    }

    preDisplayPanel = gameObject;
    ...
    healthText.text = basicAttributes.health.ToString("F2");
    ...
    if (DataBaseManager.Instance.TryGetCardIllustration(creatureAttribute.creatureCardType, out var illustration)) {
      ...
      illustrationImage.gameObject.SetActive(value: false);
    }
  }
